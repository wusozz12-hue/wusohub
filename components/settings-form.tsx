...
export function SettingsForm({ me }: { me: CurrentProfile }) {
  ...
  function pickAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > MAX_IMAGE_BYTES) {
      setError(
        `Profil fotoğrafı en fazla ${MAX_IMAGE_MB} MB olabilir.`
      )
      return
    }
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-4 py-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Profil ve uygulama ayarlarını yönet.
      </p>
          ...
          <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Küfürlü yorumları görmeye izin ver
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className={
                allowProfanity
                  ...
                  : "rounded-lg border border-border px-3 py-2 text-sm hover:bg-accent"
              }
            >
              {allowProfanity ? "Açık" : "Kapalı"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-border pt-6">
        <button
          type="button"
          onClick={signOut}
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-destructive/40 px-4 py-2.5 text-sm font-medium text-destructive transition-colors hover:bg-destructive/10"
        >
